...
namespace FenXianRouteFromD
{
    public partial class Form1 : Form
    {
        ...
        {
            string 日期 = ConfigurationManager.AppSettings["date"];
            this.Text = 日期;
            E = AddR;
            new Thread(() =>
              {
                  ...
                  {
                      Thread.Sleep(5000);
                      string bzsql = "select 标记 from tblRouteFormDBag where 日期 ='"+ 日期 + "'";
                      var bzData = SqlHelper.ExecuteDataTable(bzsql);
                      if (bzData.Rows.Count == 0)
                      {
                          string insertSql = "insert into  tblRouteFormDBag(日期,标记)VALUES('"+ 日期 + "',0)";
                      ...
                      string formx = "select 流水号,CONVERT(varchar(10),日期,120) 日期,线路ID," +
                         ...
                         "里程表数  from  tblRouteRFormX"+ Convert.ToDateTime(日期).ToString("yyyyMMdd") + "  where 线路ID!='' and 流水号>" + bz;
                      ...
                      string updatesql = "update tblRouteFormDBag set  标记=" + bz + " where  日期='"+ 日期 + "'";
